...
from plotlet.artists import _to_pylist, _marker_at
...
def _expand_err(err, n):
    ...
    if isinstance(err, tuple) and len(err) == 2:
        lo = _to_pylist(err[0]); hi = _to_pylist(err[1])
        if len(lo) == 1: lo = lo * n
        if len(hi) == 1: hi = hi * n
        return lo, hi
...
def errorbar_draw(a, ctx):
    ...
    for x, y, dxl, dxh, dyl, dyh in zip(xs, ys, xlo, xhi, ylo, yhi):
        ...
        out.append(_marker_at(marker, px, py, msize, col, 1))
    return "".join(out)
...
def errorbar_legend_swatch(a, ctx, x0, y_mid):
    ...
    return (
        ...
        + _marker_at(a["opts"].get("marker", "o"), cx, y_mid, msize, col, 1)
    )
