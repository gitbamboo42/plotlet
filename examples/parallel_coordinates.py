...
def parallel_coords_record(args, kw):
    rows = [list(_to_pylist(r)) for r in args[0]]
...
def parallel_coords_draw(a, ctx):
    ...
    for i, name in enumerate(var_names):
        ...
        out.append(_text_path(name, x, y_bot + 14, 10, anchor="middle"))
        out.append(_text_path(f"{col_hi[i]:.2g}", x + 3, y_top + 4, 8, anchor="start"))
        out.append(_text_path(f"{col_lo[i]:.2g}", x + 3, y_bot - 1, 8, anchor="start"))
    return "".join(out)
