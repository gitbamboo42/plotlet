...
from plotlet.artists import _to_2d_pylist, _to_pylist
from plotlet.colormaps import colormap, _ContinuousNorm
from plotlet._spec import _D


def bubble_record(args, kw):
    x_cats = _to_pylist(args[0])
    y_cats = _to_pylist(args[1])
    size_m = _to_2d_pylist(args[2])
    color_m = _to_2d_pylist(args[3]) if len(args) > 3 and args[3] is not None else size_m
