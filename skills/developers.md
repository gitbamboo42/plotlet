...
- **Core vs extensions split.** Core covers the **standard plotting vocabulary** (the set listed in [docs/PHILOSOPHY.md](docs/PHILOSOPHY.md)). **Domain-specific plot types** default to the separate [`plotlet-extensions`](https://github.com/gitbamboo42/plotlet-extensions) package (single-file) or the [`plotlet-cookbook`](https://github.com/gitbamboo42/plotlet-cookbook) repo (multi-file projects). Only a few extensions the core tests depend on (`numeric_bar`, `curved_tree`, `annotation_strip`, `chord_links`, `chord_ribbon`) still ship in core under [`src/plotlet/extensions/`](src/plotlet/extensions/); `plotlet.extensions` is a namespace package shared by both distributions. Decision rule for borderline cases: a plot type qualifies as "standard vocabulary" if **two or more** of matplotlib / seaborn / ggplot2 ship it as a primitive. Domain idioms (Sankey, Manhattan, ROC, KM, mosaic, calendar heatmap, etc.) don't qualify even if one popular lib has them. (Internal-only framing; not for public docs.)
