...
def diag_share_pair():
    # (a / c) | b with share_x — a-c gap collapses to 0; only a-b and c-b labeled.
    xs = _xs()
    sin = [math.sin(t) for t in xs]
    cos = [math.cos(t) for t in xs]
    a = pt.chart(title="a", data_width=290, data_height=140); a.line(xs, sin)
    c = pt.chart(title="c", data_width=290, data_height=140, share_x=a)
    c.line(xs, [s + co for s, co in zip(sin, cos)])
    b = pt.chart(title="b", data_width=290, data_height=280); b.line(xs, cos)
    return pt.layout_diagram((a / c) | b)


def diag_multi_share_grid():
    # 2x2 grid with main↔top (share_x) and main↔tree (share_y) — all joins,
    # no labeled gaps; verifies the gap detector filters joined joints.
    main = pt.chart(title="main", data_width=440, data_height=260)
    main.line([1, 2, 3, 4, 5], [2, 4, 1, 5, 3])
    top  = pt.chart(title="top",  data_width=440, data_height=80,  share_x=main)
    top.line([1, 2, 3, 4, 5], [1, 1, 3, 1, 1])
    tree = pt.chart(title="tree", data_width=120, data_height=260, share_y=main)
