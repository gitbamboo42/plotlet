...
def test_typo_kwarg_raises_at_render():
    c = pt.chart()
    c.bar(data={"x": ["a", "b"], "y": [1.0, 2.0]}, x="x", y="y", widht=0.5)
...
def test_typo_kwarg_raises_for_fanout_artist():
    c = pt.chart()
    c.line(data={"x": [1.0, 2.0], "y": [3.0, 4.0]}, x="x", y="y", width=1.5)
    with pytest.raises(TypeError, match="width"):
        c.to_svg()


def test_positional_table_still_hoists():
    # `c.bar(df, x=, y=)` sugar: replay moves the single positional
    # table into data= before the record function runs.
    c = pt.chart()
    c.bar({"x": ["a", "b"], "y": [1.0, 2.0]}, x="x", y="y")
    assert c.to_svg()
...
def test_recorder_exposes_real_signature():
    # `c.bar?` / help(c.bar) reach the record function's parameter list.
    params = inspect.signature(pt.chart().bar).parameters
    assert "fill" in params and "orientation" in params


def _text_svg(**style):
    c = pt.chart(xlim=(0, 2), ylim=(0, 2))
    c.text(data={"x": [1.0], "y": [1.0], "s": ["Ag"]}, x="x", y="y",
           label="s", **style)
    return c.to_svg()
...
def test_scatter_alpha_applies_on_colormap_path():
    ...
    opaque = pt.chart(); opaque.scatter(data=df, x="x", y="y", color="z")
    faded = pt.chart(); faded.scatter(data=df, x="x", y="y", color="z", alpha=0.3)
