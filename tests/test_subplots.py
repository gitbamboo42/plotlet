...
def share_y_collapses_gap():
    ...
    hm   = pt.chart(title="hm",   data_width=220); hm.line([1,2,3], [1,4,9])
    tree = pt.chart(title="tree", data_width=220); tree.line([1,2,3], [3,1,2])
    return (tree | hm).share_y()


def share_x_collapses_gap_vertical():
    # share_x → joined, gap 0.
    main = pt.chart(title="main", data_height=120); main.line([1, 2, 3], [1, 4, 9])
    top  = pt.chart(title="top",  data_height=120); top.line([1, 2, 3], [3, 1, 2])
...
def share_x_three_panels():
    ...
    main = pt.chart(title="main", data_height=60); main.line([0, 5, 10], [0, 1, 0])
    mid  = pt.chart(title="mid",  data_height=60); mid.line([0, 5, 10], [10, 0, 10])
    top  = pt.chart(title="top",  data_height=60); top.line([0, 5, 10], [5, 5, 5])
    return (top / mid / main).share_x()


def share_y_chain():
    # Three side-by-side panels all share y via parent-level .share_y().
    a = pt.chart(title="a", data_width=130); a.line([1,2,3], [0, 100, 0])
    b = pt.chart(title="b", data_width=130); b.line([1,2,3], [10, 50, 90])
    c = pt.chart(title="c", data_width=130); c.line([1,2,3], [20, 40, 60])
...
def width_hint_narrow_side():
    ...
    main = pt.chart(title="main", data_width=440); main.line([1,2,3], [1,4,9])
