...
def test_hist2d_validation():
    df = {"x": [1, 2], "y": [1, 2]}
    c = pt.chart(df, aes(x="x", y="y"))
    c.add_hist2d(bins=5, binwidth=0.5)
...
def test_hist2d_cell_color_matches_legend_norm():
    ...
    df = {"x": [0.5], "y": [0.5]}
    c = pt.chart(df, aes(x="x", y="y"))
    c.add_hist2d(bins=([0, 1], [0, 1]), vmin=0, vmax=2)
...
def test_hist2d_binwidth_pair():
    ...
    c = pt.chart(df, aes(x="x", y="y"))
    c.add_hist2d(binwidth=(0.5, 1.0), binrange=((0, 2), (0, 3)))
    svg = c.to_svg()
