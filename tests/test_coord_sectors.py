...
def sectors_multi_track_share_x():
    ...
    t1 = pt.chart(df, aes(x="t", y="v"),
                  data_width=480, data_height=80,
                  ylabel="scatter", ylim=(0.0, 1.0))
    t1.add_scatter(size=8)
    ...
    t2 = pt.chart(df2, aes(x="t", y="v", group="phase"),
                  data_width=480, data_height=80,
...
def sectors_from_sectors_value():
    ...
    c = pt.chart(df, aes(x="x", y="y", group="grp"),
                 title="prebuilt Sectors",
...
def sectors_continuous_y():
    ...
    c = pt.chart(df, aes(x="v", y="val"),
                 title="continuous y-sectors",
...
def test_sectors_typo_in_data_raises_clearly():
    ...
    c = pt.chart(df, aes(x="t", y="v"), data_width=200, data_height=80)
...
def test_sectors_after_artist_still_remaps():
    ...
    c = pt.chart(df, aes(x="x", y="y"), data_width=200, data_height=80, ylim=(0, 1))
    c.add_scatter()
...
def test_layout_sectors_cascade_no_leaf_mutation():
    ...
    t1 = pt.chart(df, aes(x="t", y="v"), data_width=200, data_height=60)
    t1.add_line()

    df2 = {"phase": ["training"], "t": [50], "v": [0.5]}

    t2 = pt.chart(df2, aes(x="t", y="v"), data_width=200, data_height=60)
    t2.add_line()
...
def test_sectors_passthrough_when_column_absent():
    ...
    c = pt.chart(df, aes(x="t", y="v"), data_width=200, data_height=80, ylim=(0, 1))
    c.sectors({"warmup": 100, "training": 500}, column="phase")
    c.add_scatter()
...
def test_sectors_requires_column_kwarg():
    ...
    c = pt.chart(df, aes(x="x", y="y", group="grp"), data_width=200, data_height=80)
...
def test_coord_sectors_baseline(name, fn, baseline_compare):
    baseline_compare("coord_sectors", name, fn().to_svg())
