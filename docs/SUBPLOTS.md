...
All three return a **parent `Chart`** — same type as a leaf. Call `.show()` (or `_repr_html_`) on the parent to render the whole SVG. A chart can be in **at most one parent**; composing a chart that already has a parent raises at composition time.
...
# Annotated heatmap — irregular grid
fig = pt.grid([[None, top,  None],
               [tree, hm,   pt.legend()]])
...
- **Dendrograms use the category scale.** A dendrogram on the left of a heatmap uses the heatmap's row category scale via `share_y` — no new mechanism.
