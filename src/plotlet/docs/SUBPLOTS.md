...
- **No `share_color=`.** Color isn't position-critical the way axes are; for shared gradients across heatmaps, pass matching `cmap` / `vmin` / `vmax` to each `image_cmap` and point one legend at any one of them.
