...
def attached_size_h(host: Chart) -> tuple[float, float]:
    ...
    sits at the host's data-area left edge), so they only grow the
    figure once cumulative width exceeds it. Above/below attachments
    keep their own left/right margins; if a top track's left margin
    exceeds the host's, its labels need extra figure space unless a
    left attachment already provides it. Returns (left_extra, right_extra)."""
    M = _M_eff(host)
    sum_left  = sum(c._canvas_width for c in host._attached_left)
    sum_right = sum(c._canvas_width for c in host._attached_right)
...
def attached_size_v(host: Chart) -> tuple[float, float]:
    ...
    M = _M_eff(host)
    sum_above = sum(c._canvas_height for c in host._attached_above)
    sum_below = sum(c._canvas_height for c in host._attached_below)
