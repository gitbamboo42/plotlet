...
def _check_values(value, seen: set, where: str) -> None:
    ...
    if isinstance(value, dict):
        ...
        if "$coord" in value:
            ...
            _check_values(value.get("kwargs", {}), seen, where)
            return
        if "$sectors" in value:
            ...
            _check_values(value["$sectors"], seen, where)
            return
        for v in value.values():
            _check_values(v, seen, where)
    elif isinstance(value, (list, tuple)):
        for v in value:
            _check_values(v, seen, where)


def _check_init(node, seen: set) -> None:
    ...
    _check_values(node.init, seen, where)


def _check_ops(node, seen: set) -> None:
    ...
    for i, op in enumerate(node.ops):
        ...
        _check_values(op["args"], seen, where)
        _check_values(op["kwargs"], seen, where)


def _check_insets(node, seen: set) -> None:
...
def validate(ir):
    ...
    for node in ir.nodes:
        ...
        _check_init(node, seen)
        _check_ops(node, seen)
        _check_insets(node, seen)
        seen.add(node.nid)

    if ir.root_nid not in seen:
