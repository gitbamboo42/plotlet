"""Linestyle resolution: matplotlib name aliases for plotlet's `_DASH` keys.
...
def _resolve_linestyle(ls):
    ...
    passes through unchanged so raw dasharray strings (`"6,3"`) and the
    short codes themselves still work.
    """
    if ls is None:
        return None
