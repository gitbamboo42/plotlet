...
| "Does the finished plot actually say what I meant?" | `print(pt.describe(c))` | One line per panel/artist — plot type, axis labels, scales, resolved `xlim`/`ylim`, per-series label/color/range — read from the SVG schema |
| Need those semantics programmatically | parse `data-plotlet-*` out of `c.to_svg()` (snippet below) | The same facts as structured attrs; schema in `docs/AI_ATTRS.md` |
| Title / label / legend overlaps or is cut off | `pt.lint(c)` | Automated edge-clip + overlap warnings — one per region pair (a crowded axis clusters into a single warning), naming the colliding labels; `str(w)` prints it |
...
Every call above lives on `pt` or the chart itself — nothing extra to
import.
