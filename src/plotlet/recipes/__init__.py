"""Vetted, ready-to-use plot recipes — importable after `pip install plotlet`.
...
usage pattern is import-for-side-effect, then call the method on a chart:
    ...
    import plotlet.recipes.volcano  # registers c.volcano(...)

    c = pt.chart()
    c.volcano(fc, pvals, labels, fc_threshold=1.0, p_threshold=0.01)
    c.save_svg("out.svg")

Every recipe also exposes a `demo()` function that returns a fully built
`pt.Chart` with synthetic data — useful as a starting point:

    from plotlet.recipes.volcano import demo
